...
<div
    id="sidebarBackdrop"
    class="fixed inset-0 z-30 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-300 ease-in-out md:hidden"></div>
...
<div id="logoutModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl text-center transform transition-all scale-95">
        <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-amber-100 text-[#700000]">
            <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
        </div>
        <h3 class="text-lg font-bold text-gray-800">Confirm Logout</h3>
        <p class="mt-2 text-xs md:text-sm text-gray-500">Are you sure you want to logout?</p>

        <div class="mt-6 flex gap-3">
            <button
                type="button"
                id="cancelLogoutBtn"
                class="flex-1 rounded-xl border border-gray-300 bg-white py-2.5 text-xs md:text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                Cancel
            </button>
            <button
                type="button"
                id="confirmLogoutBtn"
                class="flex-1 rounded-xl bg-[#700000] py-2.5 text-xs md:text-sm font-semibold text-[#FFD700] hover:bg-[#850000] transition">
                Logout
            </button>
        </div>
    </div>
